...
public class Test7_RedirectServlet extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// => 현재 서블릿 주소 : http://localhost:8080/StudyServlet/redirectServlet?id=ssm&passwd=1234
		// 	  따라서 현재 위치에서 test7_redirect_result.jsp를 지정하면 webapp 폴더의 파일지정
		response.sendRedirect("test7_redirect_result.jsp");
		
		// Redirect 방식 포워딩 특징
		// 1. 포워딩 시 지정한 주소가 웹브라우저 주소표시줄에 표시됨(= 요청받은 새 주소로 바뀜)
		// => http://localhost:8080/StudyServlet/redirectServlet?id=ssm&passwd=1234
		// 	  http://localhost:8080/StudyServlet/test7_redirect_result.jsp\
