...
<body>
	...
	<table border="1">
		...
		<%-- BoardDTO 객체에 저장된 멤버변수 값(데이터)을 테이블에 표시(EL을 통해 ${board.xxx} 사용 --%>
		<tr>
			<td>${board.idx }</td>
			...
			<td>${board.name }</td>
			<%-- 
			JSTL의 포맷 관련 기능을 제공하는 라이브러리를 통한 날짜 형식 변경
			=> 반드시 taglib 를 통해 JSTL의 fmt 라이브러리 추가 필수!
			=> <fmt:formatDate value="${날짜객체}" pattern="날짜패턴">
			ex) <fmt:formatDate value="${board.date }" pattern="yy-MM-dd"/>
				(날짜 패턴은 SimpleDateFormat 에서 사용하는 패턴 문자열과 동일함)
				=> board 객체의 date 날짜 데이터를 "연도2자리-월 2자리-일 2자리"로 변경
			 --%>
