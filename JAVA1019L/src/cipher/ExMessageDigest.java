...
public class ExMessageDigest {
	...
	public static void main(String[] args) {
		...
		// 새 패스워드도 단방향 암호화를 수행한 후 암후화 된 DB 패스워드와 비교해야함
...
class MyMessageDigest {
	private MessageDigest md;		// 암호화 수행 클래스 타입 변수
...
//		System.out.println("평문 : " + strSourceData);
		...
		try {
			// java.security.MessageDigest 클래스의 static 메서드의 getInstance() 메서드를 호출하여
			...
			// => 해싱 결과값(=결과값)이 byte[] 타입으로 리턴(기본 32byte 짜리 암호문 생성됨)
