...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * 	 => 주로 GUI 구현 시 이벤트 처리를 수행하는 핸들러 클래스 정의 시 사용 (= 안드로이드)
		 * - 외부 클래스와 내부 클래스로 구분됨
		 * - 클래스를 정의하는 위치에 따라 다음과 같이 구분됨
		 *  1) 멤버 내부클래스
		 *  	- 멤버변수 및 메서드와 동일한 레벨에 정의한 클래스
		 *  	- *.class 파일 생성시 외부클래스명$내부클래스명.class 형태로 생성됨(bin 폴더에)
		 *  2) 정적 내부클래스
		 *  	- 정적 멤버변수 및 메서드와 동일한 방식으로 정의한 클래스
		 *  	- 클래스 파일은 멤버 내부클래스와 동일함
		 *  3) 로컬 내부클래스 (메서드 안에 만들어진 클래스)
		 * 		- 클래스 내의 메서드 내에서 정의한 클래스
		 * 		- 특정 메서드 내에서만 사용할 클래스를 정의할 때
		 * 		- 메서드 호출 시점에서 메모리에 로딩이되므로 메서드 외부에서 접근 불가능하며
		 * 		- 메서드 내에서도 클래스 정의 코드 아래쪽에서 접근 가능함
		 */
		
		//클래스 내의 인스턴스 멤버(변수, 메서드)에 접근하려면
		...
		//=> 내부 클래스에 접근하기 위해서는 반드시 외부클래스를 통해 접근해야하는데
...
class Outer { //외부 클래스
	...
	public void method() { 	// 멤버 메서드
		//멤버 메서드 내에서는 인스턴스 멤버변수 및 메서드에 대해 자유롭게 접근 가능함
...
//		LocalInner localInner; 오류발생! => 아직 존재하지 않는 클래스
	...
	// 내부 클래스 (=> 인스턴스 멤버 내부클래스 = 멤버 내부클래스)
	class Inner {
		// 멤버 내부클래스 내에서 외부클래스의 멤버에 자유롭게 접근 가능함
		// => 멤버 메서드와 접근 권한이 동일함
