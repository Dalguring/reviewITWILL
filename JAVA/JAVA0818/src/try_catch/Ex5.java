...
public class Ex5 {
	...
	public static void main(String[] args) {
		 ...
		 *   반드시 finally 블록의 코드를 실행한 후 메서드가 종료된다.
		 * - 주로, 데이터베이스 작업(JDBC)이나 파일 작업을 위해 DB서버등의 자원 사용 시
		 *   예외가 발생하더라도 자원을 반환하기 위한 코드는 무조건 실행되어야 하므로
		 *   finally 블록 내에 자원 반환 코드를 기술하면 예외발생 여부와 관계없이 실행됨
		 * -------------------------------------------------------------------
		 * < 기본 문법 >
		 * try {
		 * 		//예외 발생 가능성이 있는 코드들
	...
	public static void method() {
		 ...
		 * 
		 * 1. 예외가 발생하지 않을 경우
		 * 		1 > 2 > return문(아직 실행 X) > 4(finally블록 실행) > return
		 * 2. 예외가 발생했을 경우
		 * 		1 > 예외발생 > 3(catch블록 실행) > 4(finally블록 실행) > 5
		 * */
