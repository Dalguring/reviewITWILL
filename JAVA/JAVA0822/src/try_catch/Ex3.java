...
public class Ex3 {
	...
	public static void main(String[] args) {
		 ...
		 *   ex) 점수입력오류를 Exception클래스 타입으로 예외 발생 시 
		 *       => InvalidScoreInputException이라는 이름을 사용하면 더 직관적이나
		 *          해당 예외 클래스는 자바에서 존재하지 않는다!
		 * - 사용자가 직접 해당이름을 가진 예외클래스를 정의 가능
		 * - 반드시 Exception 클래스(또는 계열)을 상속받아 예외클래스를 정의하여
		 *   생성자 내에서 슈퍼클래스에 예외 메시지를 전달하여 메세지 초기화 수행
		 * */
		int score = 185;
