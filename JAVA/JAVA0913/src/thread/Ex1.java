...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * - 멀티태스킹(Multi Tasking)
		 * 	 => 프로세스가 여러개일 때 해당 프로세스들이 동시에 수행되는것
		 * 		(정확히는 CPU가 빠른 속도로 프로세스들을 번갈아가면서 처리함 => 진짜 동시 X)
		 * 	 ex) 동영상을 재생하면서 웹페이지를 표시하고, 자바 코드 실행하는 것
		 * 
		 * 쓰레드(Thread)
		 * - 프로세스 내에서 작업의 최소 단위
		 * - 하나의 프로세스 내에 쓰레드가 한 개(=Single Thread)일 때
		 * 	 해당 프로세스 내에서 동시에 수행 가능한 작업은 단 하나 뿐이다.
		 * - 하나의 프로세스 내에서 동시에 수행 가능한 작업을 늘리려면
		 * 	 멀티 쓰레딩(=Multi Threading)을 구현해야한다!
		 * 	 ex) 메신저에서 파일전송과 함께 메세지 송신, 수신을 동시에 수행하는 것
		 * 
