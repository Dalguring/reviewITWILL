...
public class Ex1 {
	...
	public static void main(String[] args) {
		 ...
		 * 		   오버라이딩을 통해 슈퍼클래스의 메서드를 변경(수정)은 할 수 없다!
		 * 		- 기본문법 : [접근제한자] final 리턴타입 메서드명 (매개변수...) {}
		 * 3) 클래스에 final이 붙은 경우
		 * 		- 마지막 클래스 => 클래스 변경 불가 => 상속 X
		 * 		  => 다른 클래스에서 해당 클래스를 상속 받을 수는 없다!
		 * 		  => 어떤 클래스 자체로 이미 완전한 클래스 기능을 수행하는 경우 상속을 금지시킬 때 사용
		...
		fmv.normalVar = 99;
//		fmv.finalVar = 30; final로 선언된 멤버변수는 값 변경이 불가!
...
//		//오류 발생! finalMethod()는 서브클래스에서 오버라이딩 안됨
...
class OtherClass {
	FinalClass fc = new FinalClass(); //사용은 가능, 상속은 불가능
}
