...
public class Ex1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("num 과 20중 큰 값 : " + Math.max(num, 20));
		System.out.println("num 과 20중 작은 값 : " + Math.min(num, 20));
		...
		//     => 또한, 원래 자지로 숫자를 되돌리기 위해 다시 곱한 값만큼 나눗셈 수행
...
//			System.out.println(Math.random()); // 0.0 <= x < 1.0
			...
			// Math.random() 결과를 원하는 자릿수 만큼 정수로 이동시키고
			// 남은 자리 숫자들을 제거
//			System.out.println((int)(Math.random() * 10));	// 0 <= x < 10
			
			// 0을 제외하고 1부터 상한값 까지의 난수 발생을 위해서는
			// 난 수 발생 결과에 +1
