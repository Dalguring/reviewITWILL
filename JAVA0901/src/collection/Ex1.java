...
//		HashSet set = new HashSet(); //일반적인 HashSet 객체 생성 방법
		...
		//String toString() : 컬렉션 객체 내의 모든 요소를 문자열로 리턴 (오버라이딩 돼있음)
...
//		System.out.println("문자열 '3.14' 추가 가능한가? " + set.add("3.14")); // 데이터타입이 다르므로 추가가능
		System.out.println("Set 객체의 모든 요소 : " + set);
		
		System.out.println("-----------------------------------------------");
		
		//boolean contains(Object o) : 특정 요소(o)가 컬렉션 객체에 포함되어 있는지 판별
		System.out.println("Set 객체의 실수 3.14가 포함되어있는가? " + set.contains(3.14));
		System.out.println("Set 객체에 정수 3이 포함되어있는가? " + set.contains(3));
		System.out.println("Set 객체에 정수 4이 포함되어있는가? " + set.contains(4));
		// '4'는 있지만 정수 4는 없으므로 false 리턴됨
		
		//boolean remove(Object o) : 특정 요소(o)를 컬렉션 객체에서 제거
		//=> 삭제 성공 시 true, 실패 시 false 리턴
		System.out.println("Set 객체내의 실수 3.14 삭제 : " + set.remove(3.14));
		System.out.println("Set 객체내의 실수 3.14 삭제 : " + set.remove(3.14));
		System.out.println("Set 객체의 모든 요소 : " + set);
		...
		System.out.println("set2 객체의 모든 요소 : " + set2); //한 건이라도 추가하면 true 리턴, 다 실패 시 false 리턴
		System.out.println("set2 객체에 set 객체 모두 추가 : " + set2.addAll(set));
		...
		//=> 데이터가 저장된 주소를 공유하는 것이 아니라 실제 요소를 복사하여 전달하므로
