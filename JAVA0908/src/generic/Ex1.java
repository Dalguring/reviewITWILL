...
//			int data = list.get(i); // 컴파일 오류 발생! Object타입 리턴 시 int 타입에 저장 불가
//			int data = (int)list.get(i); // 런타임 오류 발생!
			// => 첫 번째 데이터는 정수이므로 int형으로 변환 가능하지만, 두 번째 데이터는 문자열이므로 int형 변환 불가능
...
//			System.out.println(data);
			...
			// => 또한 각 데이터타입 변수에 저장할 떄는 추가적으로 형변환이 필요함
