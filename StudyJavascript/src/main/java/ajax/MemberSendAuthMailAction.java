...
//		System.out.println(id + " " + email);
		...
		// GenerateUserAuthenticationCode 클래스 인스턴스 생성 및 Getter 호출을 통해 인증코드 리턴받기
...
//		System.out.println("인증코드 : " + authCode);
		...
		// 인증 메일 발송을 위해 SendMail 클래스 인스턴스 생성 및 sendMail() 메서드를 호출하여 메일 발송
		...
		// registAuthInfo() 메서드를 호출하여 인증 정보를 DB에 등록 작업 요청
		// => 파라미터 : 아이디, 인증코드		리턴타입 : boolean(isRegistSuccess)
		...
		// => 파리미터 : 아이디, 이메일
		// 아니면, MemberJoinResult.me 서블릿 요청(Redirect)
