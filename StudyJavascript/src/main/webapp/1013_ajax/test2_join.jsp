...
<script type="text/javascript">
	...
	// => 각 항목 체크 완료 시 해당 값을 true로 변경하고, 실패 시 false로 변경
	...
	// 2. 아이디에 대한 입력값 검증 : 4 ~ 16자리 영문자, 숫자, 특수문자(-_.)
	function checkId(id){
		let regex = /^[\w-.]{4,16}$/;
	...
	// 3. 비밀번호에 대한 입력값 검증 : 8 ~ 20자리 영문자, 숫자, 특수문자(!@#$%^&*)조합
...
// 			$("#checkPasswdResult").html("사용 가능한 비밀번호");
// 			$("#checkPasswdResult").css("color", "green");

			// 패스워드 복잡도 검사 추가 => 패스워드 부분 검사
			// 영문 소문자, 대문자, 숫자, 특수문자 조합에 대한 점수 계산 후
	...
	function checkForm() {
		...
		// => 단 각 체크항목에 대한 판별을 위해 전역변수로 체크할 변수 선언 필요
		...
		if(!checkNameResult) {
			...
			alert("비밀번호 항목 입력값 체크 필수!");
			$("#passwd").select();
			return false; // 현재 폼의 submit 동작을 중단하기 위해 false 리턴
		} 
		
		// 위의 모든 조건이 해당되지 않으면(= 모두 true이면) submit 동작 수행
		return true;

		// jQuery를 사용하여 대상 폼 객체에 대해 submit() 메서드를 호출해도 동일한 동작 수행
