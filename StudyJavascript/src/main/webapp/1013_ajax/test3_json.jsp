...
// 			}); 
			...
			// success: 와 유사한 done()으로 동일한 처리도 가능함(기술하는 위치가 달라짐)
// 			$.ajax({
// 				type: "GET",
// 				url: "json_data_test3.txt",
// 				dataType: "text", // 응답 데이터가 일반 텍스트 형식으로 전달되도록 text 타입 지정
// 			})
// 			.done(function(msg) { // 요청 성공 시(success와 동일하지만 호출 우선순위가 낮음)
...
// 			}); 
			...
			.done(function(data) { // 요청 성공 시(success와 동일하지만 호출 우선순위가 낮음)
// 				$("#resultArea").html(data); // JSON 객체로 전달받으므로 그대로 사용 불가능(출력 X)
...
// 				$("#resultArea").append("<h3>타입 : " + typeof(strData) + "</h3>"); // object
				...
				// 해당 배열 내에서 중괄호 {}로 감싸져 있는 객체에 접근해서 객체의 속성 지정
...
// 				$("#resultArea").html(data[0].id); // 첫번째 배열에 저장된 객체의 id 속성값 출력
				...
				for(let i = 0; i < data.length; i++) {
					// JSON 객체내의 배열 i번 인덱스에서 객체를 통해 속성값에 접근
					let id = data[i].id;
					...
					$("#resultArea > table").append(
						"<tr><td>" + id + "</td>" + "<td>" + name + "</td>" + "<td>" + age + "</td>" + "<td>" + agreeRcvSpam + "</td>"
					);
				}
			})
			.fail(function() { // 요청 실패 시
