...
<script type="text/javascript">
	/*
	사용자 정의 함수
	- 자바스크립트에서 제공하는 함수(내장함수)외에 개발자가 직접 정의하는 함수
	- 함수(function)는 단독으로 존재할 수 있지만, 메서드는 클래스 내에서만 정의 가능
	...
	*/
	...
	// 1. 파라미터, 리턴값도 없는 함수
	// => 파라미터가 없으므로 함수명 sister1뒤의 소괄호 안에 아무 변수도 명시하지 않음
	...
	// 1. 파라미터, 리턴값도 없는 함수 호출
	// => 파라미터가 없으므로 호출 시 함수명 sister1 뒤의 소괄호 안에 아무 데이터도 전달하지 않음
	document.write("동생아! 불 좀 꺼라!<br>");
	sister1(); //sister1()함수 호출 => 코드의 흐름이 함수쪽으로 일시적 이동(제어권이 넘어감)
	// => funtion sister1(){}함수쪽으로 코드의 흐름이 이동함
