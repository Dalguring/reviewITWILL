...
public class Ex2 {
	...
	public static void main(String[] args) {
		...
		// 2) substring(int beginIndex, int endIndex) : 시작 인덱스부터 끝 인덱스-1까지 추출
		System.out.println("s1.substring(5, 12) : " + s1.substring(5, 12));
		// => 5번 인덱스부터 12-1 (11번)까지 추출되므로 Program 추출됨
		
		// concat() : 문자열 결합 (결합연산자(+) 보다 연산 속도가 빠르다)
		System.out.println("s1.concat(!) : " + s1.concat("!"));
		
		// indexOf() : 문자열의 앞 쪽(첫 인덱스)부터 찾고자 하는 문자 또는 문자열의 인덱스 리턴
		...
		// lastIndexOf() : 문자열의 뒷 쪽(끝 인덱스)부터 찾고자 하는 문자 또는 문자열의 인덱스 리턴
		// => 탐색을 거꾸로 할 뿐, 인덱스 번호 자체가 바뀌지는 않는다!
		...
		// 따라서 "."기호 대신 "\\."기호를 사용해야만 문장 단위 분리가 가능하다
		String[] strArr2 = s4.split("\\.");
		System.out.println(Arrays.toString(strArr2));
		
		// -------------------------------------------------------
		// *contains() 메서드 : 특정 문자열 포함 여부 확인 (boolean 타입 확인)
