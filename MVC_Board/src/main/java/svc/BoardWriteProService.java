...
// Action 클래스로부터 요청을 받아 DAO 클래스와 상호작용을 통해 실제 DB작업을 수행하는 클래스
public class BoardWriteProService {
	...
	public boolean registBoard(BoardBean board) {
		...
		//	  DAO가 아닌 Service 클래스가 Connection 객체를 관리함
		...
		// 6. 작업 처리 결과에 따른 트렌젝션 처리
		if(insertCount > 0) {	// 작업 성공했을 경우
			// Insert 작업이 성공했을 경우 트렌젝션 처리(적용 = commit)을 위해
			// JdbcUtil 클래스의 commit() 메서드를 호출하여 commit 작업수행
			JdbcUtil.commit(con); // Service 클래스가 관리하는 Connection 객체 필요
			
			// 작업 처리 결과를 성공으로 표시하여 리턴하기 위해 isWriteSuccess를 true로 변경
			isWriteSuccess = true;
		} else {				// 작업 실패했을 경우
			// Insert 작업이 실패했을 경우 트렌젝션 처리의 취소를 위해
			// JdbcUtil 클래스의 rollback() 메서드를 호출하여 rollback 작업수행
			JdbcUtil.rollback(con);
		}
		
		// 7. Connection Pool로부터 가져온 Connection 객체 반환(공통)
		// => 주의! DAO 객체 내에서 Connection 객체를 반환하지 않도록 해야한다!
		JdbcUtil.close(con);
		
		// 8. 작업 요청 처리 결과 리턴
		return isWriteSuccess; // BoardWriteProAction으로 리턴
	}
}
