...
try {
	...
	// 메일 발송 프로토콜 정보 설정
	// 메일 전송에 필요한 정보를 Properties 객체에 추가 => put() 메서드 활용
	...
	// => Session클래스의 getDefaultInstance() 메서드를 호출하여 파라미터로 서버정보와 인증 정보 객체 전달
	// => 주의! JSP 내장 객체인 Session 객체와 별개의 객체이므로 변수명은 session이 아닌 다른 이름 사용 필수!
	Session mailSession = Session.getDefaultInstance(properties, authenticator);

	// 메일 발송 정보를 관리할 javax.mail.internet.MimeMessage 객체 생성
	// => 파라미터로 서버 정보와 인증정보를 관리하는 javax.mail.Session 객체 전달
	...
	// => 단, 스팸 메일 방지 정책으로 인해 상용 메일 사이트에서는 발신자 주소 설정(변경) 불가능
	...
	// 3-6) 메일 전송 날짜 및 시작 정보 설정(java.util.Date 객체를 활용하여 현재 서버의 시각 정보 전달)
