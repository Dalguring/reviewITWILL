...
<script type="text/javascript">
	...
	2. 변수명.exec(검사할 문자열) 형태로 유효성 검사
	   => 검사 결과를 true/false 형태로 리턴
	   	  if문 내에 문자열 등의 어떤 값이 있으면 무조건 true로 취급, null은 false로 취급됨
	*/
	function checkName(name) {
	   // 정규표현식에 사용될 패턴 문자열을 변수 regex에 저장
...
// 	   alert(regex + " : " + typeof(regex));
	...
	function checkPhone1(phone1) {
		...
		// 가운데자리 입력란 커서 요청 및 입력 데이터 블럭 지정
	...
	function checkPhone(phone) {
		// 전화번호 전체 검증 양식
		// 1. 앞자리 번호 3자리 숫자(단 010,011,051,02만 가능)
		// 2. 앞자리 번호 뒤에 하이폰(-) 필수
		// 3. 가운데자리 번호는 3~4자리 숫자
		// 4. 가운데자리 번호 뒤에 하이폰(-) 필수
		// 5. 마지막자리 번호 4자리 숫자
		...
		if(!regex.exec(phone)) {
			...
			// => 숫자가 아닌 데이터를 지정하는 패턴 작성 : [^0-9] 또는 \D(주의! 소문자 d는 숫자)
...
// 		let regex = /^[A-Za-z0-9-_.]+@[A-Za-z0-9-]+\.[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*$/;
		let regex = /^[\w_.]+@[\w-]+\.[\w-]+(\.[\w-]+)*$/; // 단 도메인에 _기호도 함께 포함됨
		if(!regex.exec(email)) {
			alert(email + " : 양식 틀림!");
		}
...
<body>
	...
	<form action="#">
	이름 <input type="text" name="name" id="name" placeholder="한글2~5글자" onchange="checkName(this.value)">
...
<!-- 	이름 <input type="text" name="name2" id="name2" placeholder="한글2~5글자" pattern="^[가-힣]{2,5}$"><br> -->
	...
	- <input type="text" name="phone2" id="phone2" onchange="checkPhone2(this.value)"><br>
	
	전화번호 <input type="text" name="phone" id="phone" placeholder="000-0000-0000 형식" onchange="checkPhone(this.value)"><br>
