...
//        StringRequest request = new StringRequest() {}; // => POST 방식 요청일 경우 객체 생성 형태
        StringRequest request = new StringRequest(
            ...
            // POST 방식 요청일 경우 Request
            // POST 방식 요청일 경우 StringRequest 객체 생성자 뒤에 메서드 오버라이딩 블럭({})을 추가하여
            // getParams() 메서드 오버라이딩 필요
            @Nullable
            @Override
            protected Map<String, String> getParams() throws AuthFailureError {
                // POST 방식으로 전달해야할 파라미터가 있을 경우
                // Map 계열 객체 생성하여 키=값 형태로 파라미터를 저장 후 리턴
                // ex) key=f5eef3421c602c6cb7ea224104795888&targetDt=20120101 데이터 전달 시
        ...
        // => RequestQueue 객체의 add() 메서더를 호출하여 StringRequest 객체 전달
    ...
    // ------------------- 로그인 기능 ------------------
    private void makeLoginRequest(String id, String passwd) {
//        Toast.makeText(this, "id : " + id + ", passwd : " + passwd, Toast.LENGTH_SHORT).show();

        // 입력받은 URL 가져와서 변수 strUrl 에 저장
        String strUrl = "http://192.168.3.129:8080/mvc_board/MemberMobileLogin.me";
//        strUrl += "?id=" + id + "&passwd=" + passwd;
...
//        StringRequest request = new StringRequest() {}; // => POST 방식 요청일 경우 객체 생성 형태
        StringRequest request = new StringRequest(
                Request.Method.POST, //요청 방식을 GET 방식으로 지정
                strUrl, // 요청 URL
                new Response.Listener<String>() { // 응답 리스너
                    @Override
                    public void onResponse(String response) { // 응답 전달 시 자동으로 호출됨
                        // 응답 메세지를 TextView 에 출력하기 위해 showMessage() 메서드 호출ㅁ
                        showMessage("[ 응답데이터 ]\n" + response);

                        // 로그인 처리 응답 문자열 판별(success or fail)
                        if(response.equals("success")) {
                            Toast.makeText(MainActivity.this, "로그인 성공!", Toast.LENGTH_SHORT).show();
                        } else if(response.equals("fail")) {
                            Toast.makeText(MainActivity.this, "로그인 실패!", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(MainActivity.this, "로그인 오류 발생!", Toast.LENGTH_SHORT).show();
                        }
                    }
                },
                new Response.ErrorListener() { // 에러 리스너
    ...
    private void showLoginDialog() {
        // 1. View 클래스의 inflate() 메서드를 호출하여 레이아웃 연결
        View dialogView = View.inflate(MainActivity.this, R.layout.login_dialog, null);

        // 2. 1번에서 생성한 View 객체의 findViewById() 메서드를 호출하여
        ...
        //    (패키지는 android.app 또는 android.appcompat.app)
        // => 파라미터 : 현재 액티비티
        ...
        // 4-2) 다이얼로그에 표시할 버튼 설정
        dialog.setPositiveButton("로그인", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                // 다이얼로그로부터 입력받은 아이디, 패스워드 가져와서 저장
                String id = etId.getText().toString();
                String passwd = etPasswd.getText().toString();

                // 만약 아이디 또는 패스워드 미입력 시 현재 작업 종료(임시)
                if(id.length() == 0 || passwd.length() == 0) return;

                // 로그인 작업 요청을 위해 makeLoginRequest() 메서드 호출
                // => 파라미터 : 아이디, 패스워드
                makeLoginRequest(id, passwd);
            }
        });

        // 4-3) 다이얼로그에 표시할 버튼 설정
        dialog.setNegativeButton("취소", null);

        // 4-4) 다이얼로그에 표시할 레이아웃(View 객체) 설정
