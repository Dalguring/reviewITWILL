...
public class Ex3 {
	...
	public static void main(String[] args) {
		...
		DesignerLogin d = new DesignerLogin();
		String result2 = d.login("admin", "1234");
		System.out.println(result2);
	}

}
//개발자와 디자이너가 수행해야하는 작업들의 공통 부분을 추상메서드로 정의하여
//기본적인 툴을 제시하면 개발자와 디자이너는 각각 별도로 독립된 작업 수행이 가능하다!
...
//   이 때, 디자이너쪽에서 넘어올 데이터는 임의의 데이터로 대체가 가능
//	 상호간에 사용할 파라미터를 미리 매겨변수로 정했기 때문
class DeveloperLogin implements LoginInterface { //백엔드
...
// 리턴되는 결과값을 임의로 실행하여 리턴 후 리턴된 데이터를 확인
class DesignerLogin implements LoginInterface {  //프론트엔드
