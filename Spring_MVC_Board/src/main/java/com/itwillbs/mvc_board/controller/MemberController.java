...
//		System.out.println(member);
		...
		// BCryptPasswordEncoder를 활용하여 해싱할 경우 Salting 기능을 통해
		// 매번 다른 결과값을 갖는 해싱이 가능하다!
		// 1. BCryptPasswordEncoder 객체 생성(spring-security-web 라이브러리 필수)
		BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
		// 2. BCryptPasswordEncoder 객체의 encode() 메서드 호출하여 해싱 결과 리턴
		//	  => 파라미터 : 평문	리턴타입 : String
...
//		MemberVO memberResult = service.loginMember(member);
		...
		// 2. member 테이블에서 id에 해당하는 패스워드 조회 후 리턴값 저장(getPasswd())
		//	  => 파라미터 : 아이디		리턴타입 : MemberVO(memberResult)
